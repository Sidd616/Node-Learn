...
export const RandomForestModel: React.FC<NodeProps<RandomForestModelData>> = ({ data }) => {
  ...
  const handlePredict = () => {
    ...
    const prediction = Object.entries(freqMap).sort((a, b) => b[1] - a[1])[0]?.[0] || 'Unknown';
  ...
  return (
    <NodeWrapper title="🌲 Random Forest Model">
      <div className="p-4 border rounded shadow bg-white">
        ...
        <button className="bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded" onClick={handlePredict} disabled={!featureKey || !labelKey}>
          Predict Label
        </button>
      </div>
    </NodeWrapper>
